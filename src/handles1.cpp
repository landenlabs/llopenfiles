...
typedef enum _SYSTEM_INFORMATION_CLASS {
    ...
    // // 180
    SystemSupportedProcessorArchitectures, // in: HANDLE, out:
...
typedef NTSTATUS(WINAPI *PNtDuplicateObject)(
    _In_ HANDLE SourceProcessHandle, _In_ HANDLE SourceHandle,
...
int Handles1::FindHandles(const PidList& pids, const NameList& names, bool closeHandle, bool terminateProc) {
    ...
    PNtQuerySystemInformation pQuerySystemInformation =
    ...
    if (NULL == pQuerySystemInformation || NULL == pQueryObject ||
            NULL == pDuplicateObject) {
    ...
    HANDLE sourceProcess = 0;
    ULONG lastPid = 0;
    processName[0] = '\0';

    HANDLE currentProcess = GetCurrentProcess();
    for (unsigned i = 0; i < pHandleInfo->HandleCount; i++) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (GetFileType(targetHandle) == FILE_TYPE_DISK) {
                if (NT_SUCCESS(pQueryObject(targetHandle, ObjectNameInformation,
                                            pNameInfo, len, NULL))) {
                    pName = nameBuffer;
        ...
        if (pName != NULL /*  && _strnicmp(pType, "File", 4) == 0 */) {
            printf("%-20.20s[%-6u]:%s ", processName, thisPid, pName);
            // printf(" ObjType:%02x Acc:%06x Attr:%06x", handle.ObjectTypeIndex,
            // handle.GrantedAccess, handle.HandleAttributes); printf("PID: %-6d\t",
            // thisPid); printf("Handle: 0x%-3x\t", handleValue); printf("Object:
            // 0x%-8X\t", object); printf("Type: %-20s\t", NULL != pType ? pType :
            // "<Unknown type>"); printf("Name: %-30s\t", NULL != pName ? pName : "");
            printf("\n");
        } else {
            fprintf(stderr, " [%u]   \r", thisPid);
        }
    }
    printf("\n");
    return 1;
